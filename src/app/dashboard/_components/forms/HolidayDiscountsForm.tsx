...
export function HolidayDiscountsForm({ productId }: { productId: string }) {
  const [entries, setEntries] = useState<HolidayEntry[]>([])
  const [loading, setLoading] = useState(false)

  // Fetch global holidays (e.g., US holidays for 2025)
  useEffect(() => {
    const fetchHolidays = async () => {
      ...
      try {
        ...
        const mapped = holidays.map((holiday: any) => ({
          id: `api-${holiday.date}`, // unique ID
  ...
  const updateEntry = (id: string, key: keyof HolidayEntry, value: any) => {
    setEntries(entries.map(entry => (entry.id === id ? { ...entry, [key]: value } : entry)))
  }
  ...
  const saveChanges = async () => {
    try {
      for (const entry of entries) {
        ...
        // Save all as new or update by date
      ...
      alert("Holiday deals updated successfully.")
    } catch (error) {
      console.error("Error saving holiday discounts:", error)
      alert("Failed to save holiday deals.")
    }
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Global holidays with preset discount. Edit as needed.
        </CardDescription>
      </CardHeader>
  ...
  <div className="w-full sm:w-1/3 md:w-1/4">
    ...
    />
        ...
        <div className="flex justify-end">
          <Button onClick={saveChanges}>Save All Deals</Button>
        </div>
      </CardContent>
    </Card>
  )
